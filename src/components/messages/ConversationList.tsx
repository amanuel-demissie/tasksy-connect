...
export const ConversationList: React.FC<ConversationListProps> = ({
  ...
  return <div className="space-y-4">
      ...
      {conversations.length === 0 ? <Card className="bg-white/95 backdrop-blur-sm border-border">
          ...
          {conversations.map(conversation => <Card key={conversation.id} onClick={() => onSelect(conversation.id)} className="mb-2 backdrop-blur-sm transition-colors cursor-pointer border-border bg-violet-800">
              <CardContent className="p-4 flex items-center space-x-4 bg-violet-800">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {conversation.lastMessage && <p className="text-sm text-neutral-600 truncate">
                      {conversation.lastMessage}
                    </p>}
                </div>
                <div className="flex flex-col items-end space-y-1">
                  {conversation.lastMessageTime && <span className="text-xs text-neutral-500">
                      {conversation.lastMessageTime}
                    </span>}
                  {conversation.unreadCount > 0 && <Badge variant="secondary" className="bg-primary text-white">
